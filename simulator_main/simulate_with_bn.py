...
while simulation_time < simulation_time_length:
    ...
    if ship_model.int.time >= (fault_time + 50) and state == 0 :
        leafnode_state_dict[SitCom] = 2 #Setter nødvendig kraft til high etter 500 sekunder med feil. 
        #Dette for å simulere at skipet trenger mer kraft for å nå en havn eller lignende.
        set_fbs()
        accident_times_for_plot.append((ship_model.int.time))

    ap_query = belief_prop.map_query(variables=[AP], evidence = {MEF:leafnode_state_dict[MEF], SitCom: leafnode_state_dict[SitCom]}, show_progress = True)
    np_query = belief_prop.map_query(variables=[NP], evidence = {MEF:leafnode_state_dict[MEF], SitCom: leafnode_state_dict[SitCom]}, show_progress = True)
...
#map_ax.SitComatter(auto_pilot.navigate.east, auto_pilot.navigate.north, marker='x', color='green')  # Plot the waypoints - Originalen. 
...
map_ax.SitComatter(wp_east, wp_north, marker='x', color='green')
map_ax.SitComatter(fbp_east, fbp_north, marker = 'o', color = 'red')
