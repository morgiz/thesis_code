...
D2L is changed according to set values, and includes demand because of wind and weather
MEF fault is introduced at 400
"""
...
env_config = EnvironmentConfiguration(
    current_velocity_component_from_north=-4, # Wind pushes ship south, south east
    current_velocity_component_from_east= 3, 
    wind_speed=10,
    wind_direction=315 * np.pi / 180 #In radians wind from NW
)
...
#leaf_updates = {400: [(ES, worst), (OP, worst), (FC, worst), (WT, worst)]}
...
while simulation_time < simulation_time_length:
    ...
    #Finds smalles distance to land and updates DL based on this
    obst_distances = []
    ...
    #compare wind speed to wi_limits and update Wi state
    if ship_model.wind_speed >= wi_limits[0]:
        leaf_states[Wi] = worst
    elif ship_model.wind_speed >= wi_limits[1]:
        leaf_states[Wi] = intermediate
    else:
        leaf_states[Wi] = best

    #Calculate current and update CU state
    current = np.sqrt( env_config.current_velocity_component_from_north**2 +
                       env_config.current_velocity_component_from_east**2 )
    if current >= cu_limits[0]:
        leaf_states[Cu] = worst
    elif current >= cu_limits[1]:
        leaf_states[Cu] = intermediate
    else:
        leaf_states[Cu] = best
...
######################     Save results    ######################
np.save("sim_results/test4/results", ship_model.simulation_results)
np.save("sim_results/test4/drawings", ship_model.ship_drawings)
np.save("sim_results/test4/obst", obst_array)
np.save("sim_results/test4/leaves", leaf_node_plt_states)
np.save("sim_results/test4/util", utility_for_plot)
np.save("sim_results/test4/modes_times_d2l", [modes_for_plot,times,d2l_for_plot])
np.save("sim_results/test4/childs", child_map)
np.save("sim_results/test4/acc_times", accident_times_for_plot)