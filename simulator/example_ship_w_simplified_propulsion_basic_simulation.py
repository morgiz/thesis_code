from models import ShipModelSimplifiedPropulsion, ShipConfiguration, EnvironmentConfiguration, \
    ...
    ThrottleFromSpeedSetPointSimplifiedPropulsion, NavigationSystem
import numpy as np
...
#Egen kode for waypoints
waypoints = NavigationSystem(route="path.txt")
...
while ship_model.int.time < ship_model.int.sim_time:
    ...
    next_waypoint_index, current_waypoint_index = waypoints.next_wpt(next_waypoint_index, ship_model.north, ship_model.east)

    desired_heading = waypoints.los_guidance(
        next_waypoint_index,
        ship_model.east,
        ship_model.north)
    ...
    rudder_angle = heading_controller.rudder_angle_from_heading_setpoint(
        heading_ref= desired_heading, #Endret til egen variabel. Gammel: desired_heading_radians,
