...
"""auto_pilot_fb = HeadingByRouteController(
    route_name="fallback_points.txt",
    heading_controller_gains=heading_controller_gains,
    los_parameters=los_guidance_parameters,
    time_step=time_step,
    max_rudder_angle=machinery_config.max_rudder_angle_degrees * np.pi/180
...
nodelist = [LOP, AP, MEF, NP, SC]
net.add_nodes_from(nodelist)

edgelist = [(AP,LOP), (MEF,AP), (SC, NP), (NP, LOP)]
...
cpt_ap = TabularCPD(
    ...
    evidence=[MEF], #Which nodes act as evidence
...
cpt_np = TabularCPD(
    ...
    variable_card=3,
    #   SC |  L   |  M  |  H  | 
    values=[[0.78, 0.12, 0.03],     #LOP = low
            [0.21, 0.68, 0.07],     #LOP = medium
            [0.01, 0.20, 0.90]],     #LOP = High
...
while simulation_time < simulation_time_length:
    ...
    if state == 0: 
        ...
        if ship_model.int.time >= (fault_time + 50) and state == 0 :
            ...
            set_fbs()
            accident_times_for_plot.append((ship_model.int.time))
            simulation_setup_fb = SimulationConfiguration(
                ...
                integration_step=time_step,
                simulation_time=simulation_time_length-ship_model.int.time)

            ship_model_fb = ShipModel(
                ship_config=ship_config,
                machinery_config=machinery_config,
                environment_config=env_config,
                simulation_config=simulation_setup_fb,
                initial_propeller_shaft_speed_rad_per_s=ship_model.ship_machinery_model.omega)

            with open('fallback_points.txt') as fb_file:
                temp = []
                for line in fb_file.readlines():
                    temp.append(line)
                temp.insert(0, f"{int(north_position)} {int(east_position)}\n")
            with open('fallback_points.txt', 'w') as fb_file:
                fb_file.writelines(temp)
            ...
            #Bør kanskje kjøre denne if-en litt senere, sånn at den fullfører iterasjonen for ship_model før den går inn i fallback-state 


            #Legger båten som har kjørt til nå i en midlertidig fil, endrer så ship-model tiil den som skal kjøre i fb state.
            #Bruker "ship_model" navnet videre så jeg slipper masse duplikat-kode. 
            #ship_model_temp = ship_model
            #ship_model = ship_model_fb
    ...
    #*********************************************************************
    ...
    elif state == 1: #Fallback state 
                # Measure position and speed
        north_position_fb = ship_model_fb.north
        east_position_fb = ship_model_fb.east
        heading_fb = ship_model_fb.yaw_angle
        speed_fb = ship_model_fb.forward_speed
        ...
        # Find appropriate rudder angle and engine throttle
        rudder_angle_fb = auto_pilot_fb.rudder_angle_from_route(
            north_position=north_position_fb,
            east_position=east_position_fb,
            heading=heading_fb
        )

        rudder_plot.append(rudder_angle_fb)

        throttle = throttle_controller.throttle(
            speed_set_point=desired_forward_speed_meters_per_second,
            measured_speed=speed_fb,
            measured_shaft_speed=speed_fb
        )

        # Update and integrate differential equations for current time step
        ship_model_fb.store_simulation_data(throttle)
        ship_model_fb.update_differentials(engine_throttle=throttle, rudder_angle=rudder_angle_fb)     
        ship_model_fb.integrate_differentials()

        #integrator_term.append(auto_pilot.navigate.e_ct_int)

        times.append(ship_model_fb.int.time)
    ...
    #*********************************************************************
        ...
        # Make a drawing of the ship from above every 20 second
        if time_since_last_ship_drawing > 30:
            ship_model_fb.ship_snap_shot()
            time_since_last_ship_drawing = 0
        time_since_last_ship_drawing += ship_model_fb.int.dt
        # Progress time variable to the next time step
        ship_model_fb.int.next_time() 
...
results_fb = pd.DataFrame().from_dict(ship_model_fb.simulation_results)

# Example on how a map-view can be generated
map_fig, map_ax = plt.subplots()
map_ax.plot(results['east position [m]'], results['north position [m]'])
#map_ax.plot(results_fb['east position [m]'], results_fb['north position [m]'], color = 'red')
...
with open('route.txt', 'r') as wp: 
    for line in wp.readlines():
        list = line.split(" ")
        wp_east.append(int(list[1]))
        wp_north.append(int(list[0]))

with open('fallback_points.txt','r') as fbp: 
    for line in fbp.readlines(): 
        list = line.split(" ")
        fbp_east.append(int(list[1]))
        fbp_north.append(int(list[0]))
...
for x_fb, y_fb in zip(ship_model_fb.ship_drawings[1], ship_model_fb.ship_drawings[0]):
    map_ax.plot(x_fb, y_fb, color='red')
