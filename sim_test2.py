...
"""
This script simulates a ship en-route which passes through two islands. Based on sim_2obst.py
No mode-change is done.
"""
...
mef_mode_params = MachineryModeParams( #Oppretter en mode for main engine failure: Mech mode uten main engine capacity.
    main_engine_capacity=0,
    electrical_capacity=diesel_gen_capacity,
    shaft_generator_state=hybrid_shaft_gen_as_offline
)
...
machinery_config = MachinerySystemConfiguration(
    ...
    machinery_operating_mode=2, #Mec mode i utgangspunktet
...
leaf_updates = {} #empty placeholder
...
while simulation_time < simulation_time_length:
    ...
    if min_dist <= obst_limits[0]: 
        leaf_states[D2L] = worst
    elif min_dist <= obst_limits[1]: 
    ...
    #Update WM based on wanted mode variable
    if wantedMode == mech: 
...
######################     Save results    ######################
np.save("sim_results/test2/results", ship_model.simulation_results)
np.save("sim_results/test2/drawings", ship_model.ship_drawings)
np.save("sim_results/test2/obst", obst_array)
np.save("sim_results/test2/leaves", leaf_node_plt_states)
np.save("sim_results/test2/util", utility_for_plot)
np.save("sim_results/test2/modes_times_d2l", [modes_for_plot,times,d2l_for_plot])
np.save("sim_results/test2/childs", child_map)
np.save("sim_results/test2/acc_times", accident_times_for_plot)